...
// Function to log user workout time
const logWorkoutTime = async (req, res) => {
  try {
    const userId = req.user._id; // Assuming you're using JWT authentication
    const { workoutTime } = req.body;

    // Save workout log
    await UserLogs.create({ userId, logType: "workout", workoutTime });

    res.status(200).json({ message: "Workout time logged successfully" });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};

// Function to handle user entrance
const userEntrance = async (req, res, next) => {
  try {
    // Implement logic to track user entrance
    // Example: Log user entrance timestamp, update user status, etc.
    let { enterUserId } = req.body;

    let objId = new ObjectId(enterUserId);
    ...
    res.status(200).json({ message: "User entrance tracked successfully" });
  } catch (error) {
    next(error);
  }
};
...
const userExit = async (req, res, next) => {
  try {
    // Implement logic to track user exit
    // Example: Log user exit timestamp, update user status, etc.
    let { exitUserId } = req.body;

    let objId = new ObjectId(exitUserId);
    let user = await UserLogs.findOne({ userId: objId })
      .sort({ logTime: -1 })
      .limit(1);

    console.log(user);
    let currentTime = new Date();
    let enteredTime = user.logTime;

    let workoutMinutes = (currentTime - enteredTime) / (1000 * 60);

    console.log(user.userId);
    await UserLogs.updateOne(
      { _id: user._id },
      { $set: { workoutTime: workoutMinutes } }
    );

    // Placeholder response
    res.status(200).json({ message: "User exit tracked successfully" });
  } catch (error) {
    next(error);
  }
};
